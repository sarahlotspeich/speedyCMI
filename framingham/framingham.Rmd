...
library(speedyCMI) ## for parametric imputation methods
library(imputeCensRd) ## for semiparametric imputation method
```
...
lognorm_plot
### Median time to hypertension (imputed)
median(lognorm_imp$imputed_data$imp[lognorm_imp$imputed_data$HYPERTEN == 0])
```
...
all_plot = weib_imp$imputed_data |> 
  ...
  ggplot(aes(x = imp, fill = HYPERTEN)) + 
  geom_density(alpha = 0.7) + 
  geom_vline(xintercept = 24, linetype = 2) + ## max observed uncensored value
...
lognorm_fit = cmi_fp_bootstrap(imputation_model = Surv(time = TIMEHYP, event = HYPERTEN) ~ SEX + AGE0 + BMI0 + SYSBP0, 
                               dist = "lognormal", 
                               analysis_model = SYSBP1 ~ SYSBP0 + imp + SEX + AGE0 + BMI0, 
                               data = to_analyze, 
                               type = "analytical", 
                               B = 10)
lognorm_fit$coeff$Coefficient[3] = "TIMEHYP"
forest_dat = lognorm_fit$coeff |>
  dplyr::rename(Estimate = Est) |> 
  dplyr::mutate(Method = "Multiple Parametric CMI", 
                LB = Estimate - 1.96 * SE, 
                UB = Estimate + 1.96 * SE) |>
  dplyr::bind_rows(forest_dat)
```
...
set.seed(333)
## Fit analysis model to each imputed dataset
lognorm_imp = cmi_sp_bootstrap(imputation_model = Surv(time = TIMEHYP, event = HYPERTEN) ~ SEX + AGE0 + BMI0 + SYSBP0,
                               analysis_model = SYSBP1 ~ SYSBP0 + imp + SEX + AGE0 + BMI0,
                               data = to_analyze,
                               integral = "tr",
                               surv_between = "cf",
                               surv_beyond = "d", 
                               B = 10)
lognorm_imp$Coefficient[3] = "TIMEHYP"
forest_dat = lognorm_imp |>
  dplyr::rename(Estimate = Est) |> 
  dplyr::mutate(Method = "Multiple Semiparametric CMI", 
                LB = Estimate - 1.96 * SE, 
                UB = Estimate + 1.96 * SE) |>
  dplyr::bind_rows(forest_dat)
```
...
lognorm_imp = cmi_fp_analytical(imputation_model = Surv(time = TIMEHYP, event = HYPERTEN) ~ SEX + AGE0 + BMI0 + SYSBP0, 
                                ...
                                data = to_analyze, 
                                boots = 0)
lognorm_fit = coefficients(summary(lm(formula = SYSBP1 ~ SYSBP0 + imp + SEX + AGE0 + BMI0, 
                                      data = lognorm_imp$imputed_data)))
rownames(lognorm_fit)[3] = "TIMEHYP"
forest_dat = lognorm_fit |>
  data.frame() |> 
  dplyr::rename(SE = Std..Error) |> 
  dplyr::mutate(Coefficient = rownames(lognorm_fit),
                Method = "Single Parametric CMI", 
                LB = Estimate - 1.96 * SE, 
                UB = Estimate + 1.96 * SE) |>
  dplyr::select(-t.value, -Pr...t..) |> 
  dplyr::bind_rows(forest_dat)
...
set.seed(333)
## Fit analysis model to each imputed dataset
lognorm_imp = cmi_sp(imputation_model = Surv(time = TIMEHYP, event = HYPERTEN) ~ SEX + AGE0 + BMI0 + SYSBP0,
                     data = to_analyze,
                     integral = "tr",
                     surv_between = "cf",
                     surv_beyond = "d")
...
forest_dat = lognorm_fit |>
  ...
  dplyr::rename(SE = Std..Error) |> 
  dplyr::mutate(Coefficient = rownames(lognorm_fit),
                Method = "Single Semiparametric CMI", 
                LB = Estimate - 1.96 * SE, 
                UB = Estimate + 1.96 * SE) |>
  dplyr::select(-t.value, -Pr...t..) |> 
  dplyr::bind_rows(forest_dat)
...
forest_plot = forest_dat |> 
  dplyr::mutate(Method = factor(x = Method, 
                                levels = c("Complete Case Analysis", 
                                           "Single Parametric CMI", "Multiple Parametric CMI", 
                                           "Single Semiparametric CMI","Multiple Semiparametric CMI", 
                                           "Naive Analysis"), 
                                labels = c("Complete Case\nAnalysis", 
                                           "Parametric\nCMI (Single)", "Parametric\nCMI (Multiple)", 
                                           "Semiparametric\nCMI (Single)","Semiparametric\nCMI (Multiple)", 
                                           "Naive\nAnalysis")),
...
```{r}
forest_dat |> 
  dplyr::mutate(Est_CI = paste0("$", round(Estimate, 2), "$ ($", round(LB, 2), "$, $", round(UB, 2), "$)")) |> 
  dplyr::select(Coefficient, Method, Est_CI) |> 
  tidyr::pivot_wider(names_from = "Method", 
                     values_from = "Est_CI") |>
  dplyr::select(Coefficient, "Complete Case Analysis", 
                "Single Parametric CMI", "Multiple Parametric CMI",
                "Single Semiparametric CMI", "Multiple Semiparametric CMI", 
                "Naive Analysis") |> 
...
for (d in c("lognormal", "weibull", "exponential", "loglogistic", "pwe")) {
  ...
  imp = cmi_fp_analytical(imputation_model = Surv(time = TIMEHYP, event = HYPERTEN) ~ FEMALE + AGE0 + BMI0 + SYSBP0, 
                          ...
                          boots = 200,
                          nintervals = 10)
  fit = cmi_fp_pool_fit(analysis_model = SYSBP1 ~ SYSBP0 + imp + SEX + AGE0 + BMI0,
                              mult_imp = imp)
  rownames(fit)[3] = "TIMEHYP"
  forest_dat = fit |>
    data.frame() |> 
    dplyr::mutate(Estimate = Est, 
                  Coefficient = rownames(lognorm_fit),
                  Method = toupper(d), 
                  LB = Est - 1.96 * SE, 
                  UB = Est + 1.96 * SE) |>
    dplyr::bind_rows(forest_dat)
