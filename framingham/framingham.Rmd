...
lognorm_fit = cmi_fp_bootstrap(imputation_model = Surv(time = TIMEHYP, event = HYPERTEN) ~ SEX + AGE0 + BMI0 + SYSBP0, 
                               ...
                               type = "analytical", 
                               B = 200)
lognorm_fit$Coefficient[3] = "TIMEHYP"
forest_dat = lognorm_fit |>
  dplyr::rename(Estimate = Est) |> 
  dplyr::mutate(Method = "Multiple Imputation Analysis", 
                LB = Estimate - 1.96 * SE, 
                UB = Estimate + 1.96 * SE) |>
  dplyr::bind_rows(forest_dat)
```
...
forest_dat = lognorm_fit |>
  ...
  dplyr::mutate(Coefficient = rownames(lognorm_fit),
                Method = "Single Imputation Analysis", 
                LB = Estimate - 1.96 * SE, 
                UB = Estimate + 1.96 * SE) |>
  dplyr::select(-t.value, -Pr...t..) |> 
  dplyr::bind_rows(forest_dat)
...
forest_plot = forest_dat |> 
  dplyr::mutate(Method = factor(x = Method, 
                                levels = c("Naive Analysis", "Multiple Imputation Analysis", 
                                           "Single Imputation Analysis", "Complete Case Analysis"), 
                                labels = c("Naive\nAnalysis", "Multiple\nImputation\nAnalysis",
                                           "Single\nImputation\nAnalysis", "Complete\nCase\nAnalysis")),
